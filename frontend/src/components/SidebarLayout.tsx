    ...
    <div className="flex min-h-screen bg-[#0A0A0A]">
      ...
      >
        <div className="flex h-16 items-center border-b border-[#1A3A43] px-4">
          {sidebarCollapsed ? (
            <span className="mx-auto text-xl font-semibold text-[#00D9FF]">{brandShort}</span>
          ) : (
            <Link href={navItems[0].href} className="text-xl font-semibold text-[#00D9FF]">
              {brand}
            </Link>
          )}
          <button
        ...
        <div className="absolute bottom-0 left-0 right-0 border-t border-[#1A3A43] p-2">
          {sidebarCollapsed ? (
            ...
            <div className="flex items-center justify-between px-2">
              <div className="truncate text-sm text-[#A7B8BD]">
      ...
      <div className={`flex flex-1 flex-col transition-all ${sidebarCollapsed ? "lg:pl-16" : "lg:pl-64"}`}>
        <header className="flex h-16 items-center gap-4 border-b border-[#1A3A43] bg-[#0A0A0A] px-4 lg:px-8">
          <button onClick={() => setSidebarOpen(true)} className="text-[#A7B8BD] hover:text-white lg:hidden">
